...
class VmAanvraagFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val binding: AddvmFragmentBinding = DataBindingUtil.inflate(
            inflater,
            R.layout.addvm_fragment,
            container,
            false
        )
        ...
        val viewModelFactory = VmAanvraagFactoryModel(
            VmAanvraagRepository(vmDao, projDao, backUpDao, contractDao)
        )
        ...
        vmAanvraagView.errorToast.observe(viewLifecycleOwner) {
            if (it) {
                Toast.makeText(
                    requireContext(),
                    vmAanvraagView.form.value!!.getError(),
                    Toast.LENGTH_SHORT
                ).show()
        ...
        vmAanvraagView.success.observe(viewLifecycleOwner) {
            if (it) {
                Toast.makeText(requireContext(), "Verzoek werd verstuurd", Toast.LENGTH_SHORT)
                    .show()
        ...
        vmAanvraagView.closeKeyBoard.observe(viewLifecycleOwner) {
            if (it) {
                ...
                Toast.makeText(
                    requireContext(),
                    "Je hebt een nieuw project toegevoegd!",
                    Toast.LENGTH_SHORT
                ).show()
            }
        }
        ...
        vmAanvraagView.vmNaamBestaatAl.observe(viewLifecycleOwner) {
            if (it) {
                Toast.makeText(
                    requireContext(),
                    "Naam van virtualmachine moet uniek zijn!",
                    Toast.LENGTH_SHORT
                ).show()
